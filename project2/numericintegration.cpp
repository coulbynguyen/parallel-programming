...
int main(int argc, char*argv[]){
  ...
  const float fullTileArea = (  ( ( XMAX - XMIN )/(float)(NUMNODES-1) )  * ( ( YMAX - YMIN )/(float)(NUMNODES-1) )  );
  ...
  for(int j = 0; j < NUMTRIES; j++){
    ...
    for(int i = 0; i < NUMNODES*NUMNODES; i++){
      ...
      volume += (base * Height(iu, iv) * fullTileArea);
  ...
  float averageVolume = totalVolume / NUMTRIES;
  cout << NUMT << "," << NUMNODES << "," << averageVolume << "," << maxPerformance << endl;
